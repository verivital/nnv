classdef TransposedConv1DLayer < handle
    % The transpose convolutional 2D layer class
    %   Contain constructor and reachability analysis methods
    % Matlab reference: 
    %   1) https://www.mathworks.com/help/deeplearning/ref/nnet.cnn.layer.transposedconvolution1dlayer.html
    ...
    methods
        ...
        function obj = TransposedConv1DLayer(varargin)           
...
%                         obj.Weights = filter_weights;
    ...
    methods
        ...
        function S = reach_star_single_input(obj, input)
            ...
            % compute output sets
            c = vl_nnconvt1d(double(input.V(:,:,:,1)), double(obj.Weights), double(obj.Bias), obj.Stride, obj.CroppingSize);
            V = vl_nnconvt1d(double(input.V(:,:,:,2:input.numPred + 1)), double(obj.Weights), 0, obj.Stride, obj.CroppingSize);         
            Y = cat(4, c, V);
    ...
    methods(Static) % parsing, get zero input pading, compute feature maps
        ...
        function L = parse(layer)
            % @layer: a transpose convolutional 1d layer from matlab deep
            % neural network tool box
            % @L : a TransposeCov1DLayer for reachability analysis purpose
            
            % author: Neelanjana Pal
            % date: 10/25/2021 
            
            
            if ~isa(layer, 'nnet.cnn.layer.TransposedConvolution1DLayer')
                error('Input is not a Matlab nnet.cnn.layer.TransposedConvolution1DLayer class');
            end
            
            L = TransposedConv1DLayer(layer.Name, layer.Weights, layer.Bias, layer.CroppingSize, layer.Stride, layer.NumInputs, layer.InputNames, layer.NumOutputs, layer.OutputNames);
                        
            fprintf('\nParsing a Matlab transposed convolutional 1d layer is done successfully');
