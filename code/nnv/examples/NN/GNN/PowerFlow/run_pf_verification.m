function results = run_pf_verification(bus_system, layer_type, epsilon, options)
% run_pf_verification - Shared helper for Power Flow GNN verification
%
% Syntax:
%   results = run_pf_verification(bus_system, layer_type, epsilon)
%   results = run_pf_verification(bus_system, layer_type, epsilon, options)
%
% Inputs:
...
%       unknown - Number of unknown nodes
...
% Date: 01/14/2026
    ...
    if verbose
        fprintf('=== %s Power Flow Verification (%s) ===\n', layer_type, bus_system);
        fprintf('Model: %s\n', model_path);
