function xdot = mariner_uuv(t,x,ui)
...
%
% This is the mariner version with some parameters changed to match those
% of the UUV sim.
%
...
% mass changed (certain, altho not in the units)
m  = 69.7; %(kg - from sim) %looks like its units are tons/1000000
%moment of inertia about the Zb-axis (yaw rotation)
Iz = 35;% (kg*m^2) % eca_a9_sabe -> inertial -> inertia -> izz (units?)
% center of mass for hydrodynamic forces (or bouyancy)
xG = 0.06;% (m) % parameter cob from the UUV sim (units?)

% These are the hydrodynamic coefficients, also called hydrodynamic
% derivatives because they are the partial derivatives of the forces and
% moment with respect to the corresponding velocities.
...
% Forces and moments (Model of Abkowitz (1964), Taylor series Expansions)
% That s where they were originally taken, then improved to the model of
% Ross et al. (2007)
% These are the lift and drag forces and moments
