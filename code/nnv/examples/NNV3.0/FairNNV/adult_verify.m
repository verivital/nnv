...
% Runner-driven: paths come from the `config` struct in caller workspace.
...
if ~exist('config', 'var')
    ...
    config.modelsDir = fullfile(scriptDir, 'models');
...
csv_counterfactual = fullfile(resultsDir, ['counterfactual_', timestampStr, '.csv']);
...
csv_individual = fullfile(resultsDir, ['individual_', timestampStr, '.csv']);
...
csv_timing = fullfile(resultsDir, ['timing_', timestampStr, '.csv']);
