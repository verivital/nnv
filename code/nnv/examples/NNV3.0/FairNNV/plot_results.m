...
%
% This script can be run standalone or called from run_fairnnv.m.
% Standalone: looks in the most recent results/<ts>/ subdir.
% Runner-driven: uses `config.outputDir` from caller workspace.
...
if ~exist('config', 'var')
    ...
    subdirs = dir(resultsRoot);
    subdirs = subdirs([subdirs.isdir] & ~ismember({subdirs.name}, {'.', '..'}));
    if isempty(subdirs)
        error('No results subdir found under %s. Run adult_verify.m first.', resultsRoot);
    end
    [~, mi] = max([subdirs.datenum]);
    config.outputDir = fullfile(resultsRoot, subdirs(mi).name);
    config.savePNG = true;
    config.savePDF = true;
end
...
counterfactual_files = dir(fullfile(resultsDir, 'counterfactual_*.csv'));
individual_files = dir(fullfile(resultsDir, 'individual_*.csv'));
timing_files = dir(fullfile(resultsDir, 'timing_*.csv'));

% Check if files exist
if isempty(counterfactual_files) || isempty(individual_files) || isempty(timing_files)
    error('CSV files not found in %s. Please run adult_verify.m first.', resultsDir);
end
...
latex_cf_filename = fullfile(resultsDir, 'counterfactual_table.tex');
...
if config.savePNG
    print(gcf, fullfile(resultsDir, 'individual_fairness_combined.png'), '-dpng', '-r300');
end
if config.savePDF
    print(gcf, fullfile(resultsDir, 'individual_fairness_combined.pdf'), '-dpdf', '-bestfit');
end
disp("Saved: individual_fairness_combined.png/pdf");
...
latex_timing_filename = fullfile(resultsDir, 'timing_table.tex');
...
%% Summary
disp(" ");
disp("======= FairNNV PLOTTING COMPLETE ==========");
disp("Generated outputs in " + resultsDir + ":");
disp("  1. counterfactual_table.tex (LaTeX table)");
disp("  2. individual_fairness_combined.png/pdf (Area plot)");
disp("  3. timing_table.tex (LaTeX table)");
