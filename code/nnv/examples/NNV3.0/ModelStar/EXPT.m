...
% yaml files
    ...
    methods
        ...
        % Load experiment data from a .mat file. The original implementation
        % used YAML (yaml.loadFile/dumpFile), which required an external
        % MATLAB-yaml package not bundled with NNV. This refactor switches
        % to native .mat so the example has zero external dependencies.
        ...
        function save(obj)
            ...
            if file_exists && ~obj.loaded_from_file && ~strcmp(obj.file, "results/test_file.mat")
                error("File " + obj.file + " already exists; not overwriting.")
            end
            if file_exists
                movefile(obj.file, obj.file + ".bak");
            elseif obj.loaded_from_file
                error("Current configuration has been loaded from file " + obj.file + " but the file does not exist.")
            end
            data = obj.data; %#ok<NASGU,PROPLC>  used by builtin save below
            builtin('save', char(obj.file), 'data');
            if file_exists
                delete(obj.file + ".bak");
            end
        end
        ...
        function adjust_extension(obj)
            ...
            if length(obj.file) < 4 || ~strcmp(obj.file(end - 3:end), ".mat")
        ...
        function plot_results(obj, args)
            ...
            if isfield(obj.data, "position")
                fig = gcf;
                % Position may be either a cell (legacy YAML form) or a
                % numeric array (current build_template form).
                if iscell(obj.data.position)
                    fig.Position = cell2mat(obj.data.position);
                else
                    fig.Position = obj.data.position;
                end
            end
