...
delete(gcp('nocreate'));
% NOTE: `dbstop if error` was removed — in -nodisplay -batch mode it
% freezes the process silently on any error instead of exiting cleanly.

expt = EXPT(expt_path);

% Diary log goes next to this script in a runtime/ subfolder.
ms_scriptDir = fileparts(mfilename('fullpath'));
ms_logDir = fullfile(ms_scriptDir, 'runtime');
if ~exist(ms_logDir, 'dir'); mkdir(ms_logDir); end
file = fullfile(ms_logDir, 'conv_expt_any_layer.log');
...
% Load network — model ships next to this script.
nn_name = char(expt.data.network);
switch nn_name
    case {"MNIST_MLP"}
        mnist_model = load(fullfile(ms_scriptDir, 'mnist_model_fc.mat'));
