classdef PadLayer1034 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = PadLayer1034(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'onnx__AveragePool_91'};
            this.ONNXParams = onnxParams;
        end

        function [onnx__AveragePool_91] = predict(this, onnx__Pad_90)
            if isdlarray(onnx__Pad_90)
                onnx__Pad_90 = stripdims(onnx__Pad_90);
            end
            onnx__Pad_90NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__AveragePool_91, onnx__AveragePool_91NumDims] = PadFcn(onnx__Pad_90, onnx__Pad_90NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {onnx__AveragePool_91}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'PadLayer1034');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'PadLayer1034'));
            end
            onnx__AveragePool_91 = dlarray(single(onnx__AveragePool_91), 'SSCB');
            if ~coder.target('MATLAB')
                onnx__AveragePool_91 = extractdata(onnx__AveragePool_91);
            end
        end

        function [onnx__AveragePool_91] = forward(this, onnx__Pad_90)
            if isdlarray(onnx__Pad_90)
                onnx__Pad_90 = stripdims(onnx__Pad_90);
            end
            onnx__Pad_90NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__AveragePool_91, onnx__AveragePool_91NumDims] = PadFcn(onnx__Pad_90, onnx__Pad_90NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {onnx__AveragePool_91}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'PadLayer1034');
...
function [onnx__AveragePool_91, onnx__AveragePool_91NumDims, state] = PadFcn(onnx__Pad_90, onnx__Pad_90NumDims, params, varargin)
...
%
% [ONNX__AVERAGEPOOL_91] = PadFcn(ONNX__PAD_90, PARAMS)
%			- Evaluates the imported ONNX network PADFCN with input(s)
%			ONNX__PAD_90 and the imported network parameters in PARAMS. Returns
%			network output(s) in ONNX__AVERAGEPOOL_91.
%
% [ONNX__AVERAGEPOOL_91, STATE] = PadFcn(ONNX__PAD_90, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = PadFcn(ONNX__PAD_90, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  ONNX__PAD_90:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
% Outputs:
% --------
% ONNX__AVERAGEPOOL_91
%			- Output(s) of the ONNX network.
%			  Without permutation, the size(s) of the outputs are:
%				  ONNX__AVERAGEPOOL_91:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
[onnx__Pad_90, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_90, params, varargin{:});
% Put all variables into a single struct to implement dynamic scoping:
[Vars, NumDims] = packageVariables(params, {'onnx__Pad_90'}, {onnx__Pad_90}, [onnx__Pad_90NumDims]);
% Call the top-level graph function:
[onnx__AveragePool_91, onnx__AveragePool_91NumDims, state] = PadGraph1030(onnx__Pad_90, NumDims.onnx__Pad_90, Vars, NumDims, Training, params.State);
% Postprocess the output data
[onnx__AveragePool_91] = postprocessOutput(onnx__AveragePool_91, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [onnx__AveragePool_91, onnx__AveragePool_91NumDims1033, state] = PadGraph1030(onnx__Pad_90, onnx__Pad_90NumDims1032, Vars, NumDims, Training, state)
% Function implementing the graph 'PadGraph1030'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Pad_90 = onnx__Pad_90;
NumDims.onnx__Pad_90 = onnx__Pad_90NumDims1032;

% Execute the operators:
% Pad:
[Vars.onnx__AveragePool_91, NumDims.onnx__AveragePool_91] = onnxPad(Vars.onnx__Pad_90, Vars.PadPadding1031, 0.000000, 'constant', dlarray([0:NumDims.onnx__Pad_90]'), NumDims.onnx__Pad_90);

% Set graph output arguments from Vars and NumDims:
onnx__AveragePool_91 = Vars.onnx__AveragePool_91;
onnx__AveragePool_91NumDims1033 = NumDims.onnx__AveragePool_91;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_90, numDataOutputs, params, varargin)
% Function to validate inputs to PadFcn:
p = inputParser;
isValidArrayInput = @(x)isnumeric(x) || isstring(x);
isValidONNXParameters = @(x)isa(x, 'ONNXParameters');
addRequired(p, 'onnx__Pad_90', isValidArrayInput);
addRequired(p, 'params', isValidONNXParameters);
addParameter(p, 'InputDataPermutation', 'auto');
addParameter(p, 'OutputDataPermutation', 'auto');
addParameter(p, 'Training', false);
parse(p, onnx__Pad_90, params, varargin{:});
...
function [onnx__Pad_90, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_90, params, varargin)
% Parse input arguments
[inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_90, 1, params, varargin{:});
anyDlarrayInputs = any(cellfun(@(x)isa(x, 'dlarray'), {onnx__Pad_90}));
% Make the input variables into unlabelled dlarrays:
onnx__Pad_90 = makeUnlabeledDlarray(onnx__Pad_90);
% Permute inputs if requested:
onnx__Pad_90 = permuteInputVar(onnx__Pad_90, inputDataPerms{1}, 4);
end

function [onnx__AveragePool_91] = postprocessOutput(onnx__AveragePool_91, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(onnx__AveragePool_91)
        onnx__AveragePool_91 = extractdata(onnx__AveragePool_91);
    end
end
% Permute outputs if requested:
onnx__AveragePool_91 = permuteOutputVar(onnx__AveragePool_91, outputDataPerms{1}, 4);
end
