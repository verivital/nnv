function [res, time, met] = verifyucf11(smpLen, verAlg, index, epsIndex, outChannels)
    ...
    fprintf("Running robustness verification on UCF11 %df dataset...", smpLen);

    % Load data
    data = readNPY(sprintf("data/UCF11/ucf11_grayscale_%df_verification_data.npy", smpLen));
    labels = readNPY(sprintf("data/UCF11/ucf11_grayscale_%df_verification_labels.npy", smpLen));
    ...
    % Load the model
    modelName = sprintf("ucf11_c3d_%df_reducedparams_%doutchannels.onnx", smpLen, outChannels);
    ...
    t = tic; % part of verification so start timer
    ...
    s_pgd = reshape(s, 1, 1, smpLen, 112, 112); % prepare sample for PGD
