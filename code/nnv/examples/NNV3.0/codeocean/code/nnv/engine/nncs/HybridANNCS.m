classdef HybridANNCS < handle
    %Hybrid Automata Neural network control system class 
    ...
    %
    % @controller: a neural net controller (FFNNS)
    % @plant: a plant model (a Hybrid Automata model: HybridA)
    % @feedbackMap: a feedback map from outputs of the plant to the input
    %           of the controller (Optional, default value -> [0])
    
    properties
        controller = []; % neural network controller (FFNNS)
        plant = []; % plant model : hybid automata (CORA wrapper)
    ...
    methods
        ...
        %constructor
        function obj = HybridANNCS(varargin)
            % @controller: a neural net controller (FFNNS)
            % @plant: a plant model (a Hybrid Automata model: HybridA)
        ...
        function [P, reachTime] = reach(obj, reachPRM)
            ...
            %
             ...
             %       5) @reachMethod: reachability method for the controller
             ...
             if ~strcmp(reach_method, 'approx-star') && ~strcmp(reach_method, 'exact-star')
                 error('Only Star methods are supported for Hybrid Automata NNCS');
             end
             
             if strcmp(reach_method, 'exact-star')
                 warning('Only approx-star method is supported for Hybrid Automata class. Plant reachability will be changed to approx-star');
             end
...
%                  input_set = obj.nextInputSetStar(fb_I{1});
                 input_set = initSet.affineMap(obj.plant.C,[]);
                 if i==2
                     lb = zeros(obj.plant.nI, 1); % at first step, U = 0.
                     U = Star(lb, lb);
                 else
                    [U,~] = obj.controller.reach(input_set, reach_method, n_cores); % control set at step i   
                 end
                 ...
                 initSet = obj.plant.stepReachStar(fb_I{1}(length(fb_I{1})), U1);
                 obj.reachSetTree.addReachSet(initSet, i);                 
             end
