...
load('mnist_networks','mnist_21_iou83');
Nets = matlab2nnv(mnist_21_iou83);
load('mnist_networks.mat', 'mnist_avg_21');
N1 = matlab2nnv(mnist_avg_21);
Nets = [Nets N1];
load('mnist_networks.mat','mnist_16');
N2 = matlab2nnv(mnist_16);
Nets = [Nets N2];
%load('test_images.mat');
poolobj = gcp('nocreate');
delete(poolobj); % reset parpool

Nmax = [1 2 3 4 5];% 40 50]; % maximum allowable number of attacked pixels 
M = length(Nmax);
L = length(Nets);

%% create input set
de = 1;%0.001; % size of input
N = 100; % number of tested images
im_folder = 'C:\Users\paln\Documents\26_Aisola2023\Semantic Segmentation\Datasets\mnist\train_images\'; 
