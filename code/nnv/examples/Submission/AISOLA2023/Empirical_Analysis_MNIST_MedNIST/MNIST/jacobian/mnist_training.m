function mnist_training(data, initializerP)
    %
    ...
    if ~strcmp(initializerP, "narrow-normal")
        layers = [imageInputLayer([28 28 1], Mean=mean(Xtrain,4))
            ...
            flattenLayer
            fullyConnectedLayer(10, "WeightsInitializer",initializerP, 'BiasInitializer', 'zeros')
            softmaxLayer];
    else
        layers = [imageInputLayer([28 28 1], Mean=mean(Xtrain,4))
            ...
            averagePooling2dLayer(2,'Stride',2,'Padding',[0 0 0 1])
            flattenLayer
            fullyConnectedLayer(10, "WeightsInitializer",initializerP, 'BiasInitializer',initializerP)
            softmaxLayer];
    end
