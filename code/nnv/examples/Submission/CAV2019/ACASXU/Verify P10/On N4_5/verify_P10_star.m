...
% safety property: The score for COC is the minimal score
% unsafe region: COC is not the minimal score: x1 >= x2; x1 >= x3;
% x1 >= x4, x1 >= x5
...
parfor i=1:n
    ...
    if isempty(S)
        fprintf('\nThe %d^th output set does not reaches the unsafe region, P10 holds', i);
    else
        fprintf('\nThe %d^th output set reaches the unsafe region, P10 is violated', i);
        safe = safe + 1;
    end
end

if safe >= 1
    fprintf('\n P10 is violated on N4_5');
else
    fprintf('\n P10 holds on N4_5');
end
