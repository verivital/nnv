function run_cnn_all()

    % Run all cnn reach experiments with subset of pixels attacked
    ...
    for noiseT = noise
        % Run small network
        reach_cnn_small(pix,numT,noiseT,XTest,YTest,cora,'random');
        % Run medium network
        reach_cnn_medium(pix,numT,noiseT,XTest,YTest,cora,'random')
        % Run tiny network
        reach_cnn_tiny(pix,numT,noiseT,XTest,YTest,cora,'random');
    end
