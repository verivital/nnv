...
save reachSet_polyhedron.mat S;

reachTime = toc(start);
% plot reachable set
% plot reachable set

S_Polyhedron = cell(1, N+1);
for i=1:N+1
    Si = S{1,i};
    m = length(Si);
    S2 = [];
    for j=1:m
        Sj = Si(j).affineMap([1 0 0; 0 1 0]);
        S2 = [S2 Sj];
    end
    S_Polyhedron{1, i} = S2;
end
...
function speed_brakes = get_speed_brakes(brake, norm_X)
    ...
    parfor i=1:n
        Ps = [speed brake(i)];
        speed_brakes = [speed_brakes Conversion.concatenatePolyhedron(Ps)];    
    end 
end
...
function [X, Y] = stepReachPlant(A, B, X0, scaled_controls)
    ...
    Y = next_X; % store the polyhedron reach sets
