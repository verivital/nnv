...
if safe >= 1
    fprintf('\nP3 is violated on N1_3');
else
    fprintf('\nP3 holds on ');
end

exact_star_safety_checking_time = toc(t) + normalized_time1;
fprintf('\nFinish verifying exact star reach set in %.4f N1_3seconds.', exact_star_safety_checking_time);
...
if isempty(S)
    fprintf('\nThe over-approximate star reach set does not reaches the unsafe region, P3 holds on N1_3');
else
    fprintf('\nThe over-approximate star reach set reaches the unsafe region, safety is unknown, P3 may be violated on N1_3');
end
...
if isempty(S)
    fprintf('\nThe over-approximate zonotope reach set does not reaches the unsafe region, P3 holds on N1_3');
else
    fprintf('\nThe over-approximate zonotope reach set reaches the unsafe region, safety is unknown, P3 may be violated on N1_3');
end
...
if isempty(S)
    fprintf('\nThe over-approximate abstract-domain reach set does not reaches the unsafe region, P3 holds on N1_3');
else
    fprintf('\nThe over-approximate abstract-domain reach set reaches the unsafe region, safety is unknown, P3 may be violated on N2_9');
end
