function [LB,UB,T,PR,POR,T_avg,T_sum] = reachabilityNoiseForSOC(varargin)
%% the function reachabilityNoiseForSOC takes the following inputs
...
%   T_sum   : total time for the reachability calculation
    ...
    for j = 1: length(input)
        ...
        LB(j) = L(end)*100;
        UB(j) = U(end)*100;
        LB_allow(j) = allowableLB{j,1}(end);
        UB_allow(j) = allowableUB{j,1}(end);
    end
