function [res, time, met] = verifyucf11(smpLen, verAlg, index, epsIndex)
    ...
    % Load data
    data = readNPY(sprintf("data/UCF11/ucf11_%df.npy", smpLen));
    labels = readNPY(sprintf("data/UCF11/ucf11_labels_%df.npy", smpLen));
    ...
    % Load the model
    modelName = sprintf("ucf11_c3d_%df.onnx", smpLen);
...
function VS = L_inf_attack(x, epsilon, numFrames)
    ...
    lb_min = zeros(numFrames, 112, 112, 3);
    ub_max = ones(numFrames, 112, 112, 3);
