for ($i = 1 ; $i -le 49; $i++) {
    ...
    while (-not $process.HasExited) {
        ...
        if ($sw.Elapsed.TotalSeconds -ge 150) {
            Write-Host "Iteration $i timed out. Killing MATLAB..."
            Stop-Process -Id $process.Id -Force
            Set-Content -Path ("Sound_results_timed_out_" + $i + ".txt") -Value "timed out"
            break
        }
    }

    if ($process.HasExited -and $sw.Elapsed.TotalSeconds -lt 150) {
