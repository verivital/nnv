for ($i = 1 ; $i -le 6; $i++) {
    ...
    while (-not $process.HasExited) {
        ...
        if ($sw.Elapsed.TotalSeconds -ge 3600) {
            Write-Host "Iteration $i timed out. Killing MATLAB..."
            Stop-Process -Id $process.Id -Force
            Set-Content -Path ("collins_results_timed_out_" + $i + ".txt") -Value "timed out"
            break
        }
    }

    if ($process.HasExited -and $sw.Elapsed.TotalSeconds -lt 3600) {
