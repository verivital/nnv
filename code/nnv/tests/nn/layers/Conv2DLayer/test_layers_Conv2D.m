...
%% %% test 4b: Conv2DLayer reach (single precision)
...
L0 = Conv2DLayer(single(W), single(b));
...
image = image.changeVarsPrecision('single');
...
%% test 5b: Conv2DLayer reach star exact (single precision input)
...
L = Conv2DLayer(single(W), single(b));
% L.set_weights_biases(W, b);
...
input = ImageStar(IM, LB, UB);
input = input.changeVarsPrecision('single');
