...
if ~is_github_actions() && gpuDeviceCount % out of memory error in github actions

    load("img3d_data.mat"); % img_X and img_I
    
    % These are the shared variables we will use throughout this script
    
    disturbance = gpuArray(ones(size(img_I)));
    
    % Constructor
    I = VolumeStar(gpuArray(img_I), -disturbance, disturbance);
    X = VolumeStar(gpuArray(img_X), -disturbance, disturbance);
