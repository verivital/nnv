...
for i=1:eqs
    for s=1:gens
        ...
        for j=1:dim
            for k=1:dim
                for l=1:dim
                    G1new(i,s) = G1new(i,s) + C{i,j}(k,l)*G(j,s+1)*G(k,s+1)*G(l,s+1);
                end
            end
        end
    end
...
for i=1:eqs
    for s=1:gens
        ...
        for j=1:dim
            for k=1:dim
                for l=1:dim
                    G2new(i,s) = G2new(i,s) + 0.5*C{i,j}(k,l)*(...
                        G(j,s+1)*G(k,s+1)*G(l,1) + ...
                        G(j,s+1)*G(k,1)*G(l,s+1) + ...
                        G(j,1)*G(k,s+1)*G(l,s+1));
                end
    ...
    for s=0:(gens-1)
        for t=(s+1):gens
            ...
            for j=1:dim
                for k=1:dim
                    for l=1:dim
                        G3new(i,counter) = G3new(i,counter) + C{i,j}(k,l)*(...
                            G(j,s+1)*G(k,s+1)*G(l,t+1) + ...
                            G(j,s+1)*G(k,t+1)*G(l,s+1) + ...
                            G(j,t+1)*G(k,s+1)*G(l,s+1));
                    end
                end
            end
            counter = counter + 1;
    ...
    for s=1:(gens-1)
        for t=(s+1):gens
            ...
            for j=1:dim
                for k=1:dim
                    for l=1:dim
                        G4new(i,counter) = G4new(i,counter) + C{i,j}(k,l)*(...
                            G(j,t+1)*G(k,t+1)*G(l,s+1) + ...
                            G(j,t+1)*G(k,s+1)*G(l,t+1) + ...
                            G(j,s+1)*G(k,t+1)*G(l,t+1));
                    end
                end
            end
            counter = counter + 1;
        ...
        for t=(s+1):(gens-1)
            for u=(t+1):gens
                ...
                for j=1:dim
                    for k=1:dim
                        for l=1:dim
                            G5new(i,counter) = G5new(i,counter) + C{i,j}(k,l)*(...
                                G(j,s+1)*G(k,t+1)*G(l,u+1) + ...
                                G(j,s+1)*G(k,u+1)*G(l,t+1) + ...
                                G(j,t+1)*G(k,s+1)*G(l,u+1) + ...
                                G(j,t+1)*G(k,s+1)*G(l,u+1) + ...
                                G(j,t+1)*G(k,u+1)*G(l,s+1) + ...
                                G(j,u+1)*G(k,s+1)*G(l,t+1));                             
                        end
                    end
                end
                counter = counter + 1;
