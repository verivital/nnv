function [completed,t,specSat,constrViol] = example_linear_reach_ARCH18_iss(varargin)
% example_linear_reach_ARCH18_iss - example for linear dynamics.
%
% Syntax:  
%    example_linear_reach_ARCH18_iss()
%
% Inputs:
%    no
%
...
%    t - computation time for reachability analysis and verification
%    specSat - boolean: true if all specifications are satisfied
%    constrViol - amount of constraint violation (negative -> constraints
%                 satisfied)
%
% Example: 
%
...
for i=1:length(Rout)
    temp = interval(project(Rout{i},3));
    
    temp2 = max(supremum(temp) -7e-4,-infimum(temp) -7e-4);
    constrViol = max(constrViol,temp2);
    
    if supremum(temp) > 7e-4 || infimum(temp) < -7e-4 
        violationSafe = 1;
        break;
    elseif supremum(temp) > 5e-4 || infimum(temp) < -5e-4
        violationUnsafe = 1;
    end  
end
