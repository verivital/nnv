...
if isfield(options,'lagrangeRem') && isfield(options.lagrangeRem,'method') && ...
   ~strcmp(options.lagrangeRem.method,'interval')
    ...
    [objX,objU] = initRangeBoundingObjects(totalInt,IHinput,options);

    % evaluate the Lagrane remainder 
    error=obj.lagrangeRemainder(objX,objU,dx,du,options.paramInt);
else
    error=obj.lagrangeRemainder(totalInt,IHinput,dx,du,options.paramInt);
end
