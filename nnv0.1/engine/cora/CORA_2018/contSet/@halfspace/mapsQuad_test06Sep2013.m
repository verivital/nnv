...
%tilde_d in zonotope form
if isnumeric(tilde_d)
    tilde_d_zono = zonotope(tilde_d);
else
    tilde_d_zono = zonotope([mid(tilde_d),rad(tilde_d)]);
end

%set-based values
Z_mat = get(R0,'Z');
nrOfGens = length(Z_mat(1,:)) - 1;
Lambda_zono = n'*(A*R0 + f);
Theta_aux_zono = (-1)*((-1)* tilde_n'*R0 + tilde_d_zono)*Upsilon; % order of summation important due to subsequent exact addition
Theta_zono = exactPlus(-tilde_n*Lambda_zono, Theta_aux_zono, nrOfGens); % use with caution!
...
if ~(infimum(Lambda_int)<0 && supremum(Lambda_int)>=0)
    ...
    k = x0 + (A*x0 + f)*(tilde_d - tilde_n'*x0)/Lambda;

    %compte linear map L
    L = I + ...
        A*(tilde_d-tilde_n'*x0)/Lambda + ...
