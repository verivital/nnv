function completed = example_linear_reach_ARCH18_iss_Uconst()
% example_linear_reach_ARCH18_iss_Uconst - example for linear dynamics with 
%                                          constant inputs.
%
% Syntax:  
%    example_linear_reach_ARCH18_iss_Uconst()
%
% Inputs:
%    no
%
...
for i=1:length(Rout)
    ...
    if supremum(temp) > 5e-4 || infimum(temp) < -5e-4 
        violationSafe = 1;
        break;
    elseif supremum(temp) > 1.7e-4 || infimum(temp) < -1.7e-4
        violationUnsafe = 1;
    end  
end
