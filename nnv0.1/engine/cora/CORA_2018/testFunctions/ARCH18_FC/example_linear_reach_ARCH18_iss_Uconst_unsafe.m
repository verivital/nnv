function completed = example_linear_reach_ARCH18_iss_Uconst_unsafe()
% example_linear_reach_ARCH18_iss_Uconst_unsafe - example for linear 
%       dynamics with constant inputs. It is checked if the unsafe
%       specification is vioulated as it is desired
%
% Syntax:  
...
% check if the spefication is satisfied
    ...
    if supremum(temp) > 1.7e-4 || infimum(temp) < -1.7e-4 
        res = 0;
    end
